...
def generate_diff_html_report(cambio, titulo_cambio, tower_user_name, output_file, dest_infoprepos):
    ...
    diff_types = [
        ...
        'networks', 'rutas', 'recursos', 'puertos', 'nameservers', 'hba', 'multipath'
    ]
...
def generate_diff_html(play_hosts, hostvars, hostvars_diff, diff_types, titulo_cambio, tower_user_name):
    ...
    for host in play_hosts:
        ...
        for diff_type in diff_types:
            ...
            tabs_html.append(
                f"<div class='file-tab' onclick=\"showFile('{host}', '{diff_type}')\">"
                f"{diff_type.replace('_', ' ').title()}"
                f"</div>"
            )
        ...
        for diff_type in diff_types:
            ...
            if diff_key in host_diff_data and 'content' in host_diff_data[diff_key]:
                ...
                content_html = (
                    f'<div id="{host}_{diff_type}" class="file-content">\n'
                    '<div class="error-message">No se encontraron datos de diferencias</div>\n'
                    '</div>'
                )
            ...
            <div style="border: 1px solid var(--border-color); border-radius: 6px; padding: 10px; background-color: #f6f8fa; font-size: 14px; text-align: left;">
                <table style="border-collapse: collapse; width: 100%;">
                    <tr>
                        ...
                        <td style="padding: 4px;">{host_data.get('ansible_distribution', 'N/A')} {host_data.get('ansible_distribution_version', 'N/A')}</td>
                        <td style="font-weight: bold; padding: 4px;">CPU:</td>
                        <td style="padding: 4px;">{host_data.get('ansible_processor_vcpus', 'N/A')} vCPUs</td>
                    </tr>
                    <tr>
                        <td style="font-weight: bold; padding: 4px;">IP:</td>
                        <td style="padding: 4px;">{host_data.get('ansible_host', 'N/A')}</td>
                        <td style="font-weight: bold; padding: 4px;">Memoria:</td>
                        <td style="padding: 4px;">{int(host_data.get('ansible_memtotal_mb', 0)) // 1024} GB</td>
                    </tr>
                    <tr>
                        <td style="font-weight: bold; padding: 4px;">Kernel:</td>
                        <td style="padding: 4px;">{host_data.get('ansible_kernel', 'N/A')}</td>
                        <td style="font-weight: bold; padding: 4px;">Arquitectura:</td>
                        <td style="padding: 4px;">{host_data.get('ansible_architecture', 'N/A')}</td>
                    </tr>
                </table>
            </div>
...
def get_host_header_data(host, cambio, dest_infoprepos):
    ...
    data = {
        'ansible_distribution': 'N/A',
        'ansible_distribution_version': 'N/A',
        'ansible_processor_vcpus': 0,
        'ansible_virtualization_role': 'guest',
        'ansible_kernel': 'N/A',
        'ansible_memtotal_mb': 0,
        'ansible_product_name': 'N/A',
        'ansible_host': 'N/A',
        'ansible_architecture': 'N/A',
        'ansible_product_uuid': 'N/A'
    }
        ...
        with open(header_file, 'r') as f:
            for line in f:
                ...
                if '=' in line:
                    ...
                    if key == 'IP':
                        data['ansible_host'] = value
                    elif key == 'SO':
                        if 'RedHat' in value:
                            data['ansible_distribution'] = 'RedHat'
                            version = value.replace('RedHat', '').replace('CentOS', '').strip()
                            data['ansible_distribution_version'] = version.split()[0] if version else 'N/A'
                        else:
                            parts = value.split()
                            data['ansible_distribution'] = parts[0] if parts else 'N/A'
                            data['ansible_distribution_version'] = parts[1] if len(parts) > 1 else 'N/A'
                    elif key == 'KERNEL':
                        data['ansible_kernel'] = value
                    elif key == 'ARCH':
                        data['ansible_architecture'] = value
                    elif key == 'CPU':
                        try:
                            data['ansible_processor_vcpus'] = int(value)
                        except ValueError:
                            data['ansible_processor_vcpus'] = 0
                    elif key == 'MEM':
                        try:
                            data['ansible_memtotal_mb'] = int(float(value))
                        except ValueError:
                            data['ansible_memtotal_mb'] = 0
                    elif key == 'TIPO':
                        data['ansible_product_name'] = value
                        if 'virtual' in value.lower() or 'vm' in value.lower():
                            data['ansible_virtualization_role'] = 'guest'
                        else:
                            data['ansible_virtualization_role'] = 'host'
                    elif key == 'UUID':
                        data['ansible_product_uuid'] = value.split()[0] if value else 'N/A'
